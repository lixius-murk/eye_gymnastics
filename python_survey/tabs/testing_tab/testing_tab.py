...
class TestingTab(QWidget):
    ...
    def _on_start(self):
        """Initialize survey and begin testing."""
        loader = SurveyLoader()
        ...
        self.current_idx = 0
        self.answers = {}  # Clear answers - now using question_id as key
    ...
    def _save_answer(self):
        """Save current question's answer with question_id as key."""
        q = self.questions[self.current_idx]
        question_id = q.get("question_id", f"q_{self.current_idx}")
        q_type = q.get("type", "single_choice")
        ...
        # Store answer by question_id (not index) for consistency
        ...
        # Use question_id as key for consistent retrieval
        self.answers[question_id] = survey_answer

    def _finish(self):
        """Complete the survey and emit results."""
        all_answers = []
        
        # Iterate through answers dict (now keyed by question_id)
